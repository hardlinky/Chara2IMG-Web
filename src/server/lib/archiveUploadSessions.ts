...
// Uploads arrive in small pieces because a single large request does not
// survive the HTTP proxy in front of this server. The client addresses each
// piece by byte offset, so it is free to shrink the size it sends.
...
export async function createArchiveUploadSession(input: {
  ...
  fileSize: number;
  fingerprint: string;
}): Promise<{ uploadId: string; chunkBytes: number; receivedBytes: number }> {
  purgeStaleSessions();
  ...
  return { uploadId: session.id, chunkBytes: ARCHIVE_UPLOAD_CHUNK_BYTES, receivedBytes: 0 };
}
...
export async function appendArchiveUploadChunk(
  ...
  // A retried piece that already landed is acknowledged, so a connection that
  // dropped after the server wrote the bytes never wedges the upload.
  if (offset + data.byteLength <= session.receivedBytes) {
  ...
  if (offset !== session.receivedBytes) {
    return {
      ok: false,
      status: 409,
      error: `Expected offset ${session.receivedBytes}`,
      receivedBytes: session.receivedBytes
    };
  }
